// The functional class for rtk_yt_localization
// Author: Yuting Xie
// 2022.2.15

#ifndef RTK_YT_LOCALIZATION_H
#define RTK_YT_LOCALIZATION_H
...
// The worker class which actually does component's job
class RTKYTLocalization {
public:
    RTKYTLocalization();
    ~RTKYTLocalization() = default;

    void InitConfig(const rtk_config::Config &config);
    void GpsCallback(const std::shared_ptr<Gps> &gps_msg);
...
private:
    void RunWatchDog(double gps_timestamp); // the watchdog mainly checks timestamps
    void PrepareLocalizationMsg(const Gps &gps_msg, LocalizationEstimate *localization, LocalizationStatus *localization_status);
    void ComposeLocalizationMsg(const Gps &)
...
private:
    std::string module_name_ = "yt_localization";
    ...
    // TODO: (yuting) implement RTKYTLocalizationTest
    // FRIEND_TEST(RTKYTLocalizationTest, InterpolateIMU);
    // FRIEND_TEST(RTKYTLocalizationTest, ComposeLocalizationMsg);
